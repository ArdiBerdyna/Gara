#!/usr/bin/env python3
"""Heuristic solver and strict local validator for the Street Cleaning problem.

Supports both the current input format (with N coordinate lines) and the older
training format used by the supplied DOCX files (without coordinates).
"""
...
INF = 10**30
# Exact DP over task subsets is only practical for small mandatory groups.
...
class Solver:
    ...
    def optional_gain(self, edge_idx: int, vehicle_idx: int) -> float:
        ...
        gain = self.ins.alpha * edge.length / max(1, self.lmax)
        if self.wmax > 0:
            gain -= (1 - self.ins.alpha) * waste / self.wmax
        return gain
    ...
    def best_insertions(
        ...
        for vehicle_idx in vehicle_indices:
            ...
            for position in range(len(routes[vehicle_idx]) + 1):
                ...
                if mandatory:
                    # Preserve scarce high-capacity vehicles and limit compulsory waste.
                    ...
                    waste = excess * edge.length / 1000
                    gain = self.ins.alpha * edge.length / max(1, self.lmax)
                    if self.wmax > 0:
                        gain -= (1 - self.ins.alpha) * waste / self.wmax
                    if gain <= 1e-15:
                        continue
                    ratio = gain / max(1, delta)
                    key = (-ratio, -gain, float(delta), float(excess))
    ...
    def add_opportunistic_nodes(
        ...
        for edge in self.ins.edges:
            ...
            for vehicle_idx, edge_ids in enumerate(traversed):
                ...
                if self.wmax > 0:
                    gain -= (1 - self.ins.alpha) * waste / self.wmax
                if gain > best_gain and gain > 1e-15:
                    best_gain = gain
    ...
    def construct_mandatory(self, restart: int) -> list[list[int]] | None:
        ...
        for edge_idx in tasks:
            ...
            if not candidates:
                return None
            # Mostly greedy, occasionally diversify among near-best choices.
            chosen_index = 0
    ...
    def refine_routes(
        self, routes: list[list[int]], cleaned: list[set[int]], rounds: int = 2
    ...
    def relocate_between_routes(self, routes: list[list[int]]) -> list[list[int]]:
        """Move service edges between vehicles when it reduces total travel time.

        Offloading expensive detours frees per-vehicle slack, which lets more
        optional streets fit on the next fill. Capacity feasibility is enforced,
        so the move never invalidates the solution.
        """
        routes = [route[:] for route in routes]
        costs = [self.route_cost(route) for route in routes]
        improved = True
        guard = 0
        max_iters = 3 if self.is_large else 8
        while improved and guard < max_iters:
            improved = False
            guard += 1
            for source in range(len(routes)):
                for edge_idx in list(routes[source]):
                    if edge_idx not in routes[source]:
                        continue
                    edge = self.ins.edges[edge_idx]
                    without = [e for e in routes[source] if e != edge_idx]
                    cost_without = self.route_cost(without)
                    removed_saving = costs[source] - cost_without
                    best: tuple[float, int, int] | None = None
                    for target in range(len(routes)):
                        if target == source:
                            continue
                        if CAPACITY[self.ins.vehicles[target]] < edge.requirement:
                            continue
                        base = costs[target]
                        route_t = routes[target]
                        for position in range(len(route_t) + 1):
                            trial = route_t[:position] + [edge_idx] + route_t[position:]
                            new_cost = self.route_cost(trial)
                            if new_cost > self.ins.time_limit:
                                continue
                            added = new_cost - base
                            delta = added - removed_saving
                            if delta < -1e-6 and (best is None or delta < best[0]):
                                best = (delta, target, position)
                    if best is None:
                        continue
                    _delta, target, position = best
                    routes[source] = without
                    routes[target].insert(position, edge_idx)
                    costs[source] = cost_without
                    costs[target] = self.route_cost(routes[target])
                    improved = True
        return routes

    def local_search(self, routes: list[list[int]]) -> list[list[int]]:
        """Combine within-route or-opt and between-route relocation."""
        passes = 2 if self.is_large else 4
        routes = [self.optimize_sequence(r, max_passes=passes) for r in routes]
        routes = self.relocate_between_routes(routes)
        routes = [self.optimize_sequence(r, max_passes=passes) for r in routes]
        return routes
    ...
    def fill_and_compact(
        ...
        """Alternate route compaction and optional insertion until it converges.

        Compacting each route frees travel time, which can let an extra optional
        edge fit on the next fill. Optional count is monotonic, so this is safe.
        """
        routes = self.local_search(routes)
        routes, cleaned = self.fill_optionals(routes, fast=fast)
        previous = sum(len(group) for group in cleaned)
        for _ in range(max_cycles):
            routes = self.local_search(routes)
            routes, cleaned = self.fill_optionals(routes, fast=fast)
            current = sum(len(group) for group in cleaned)
            if current <= previous:
                break
            previous = current
        return routes, cleaned

    def optimize_sequence(self, sequence: list[int], max_passes: int = 4) -> list[int]:
        """Reduce a vehicle's closed-tour time with or-opt relocation moves.

        Only reorders service edges, so feasibility (time limit) is preserved:
        a cheaper ordering of an already-feasible route is still feasible.
        """
        if len(sequence) <= 2:
    ...
    def add_opportunistic(
        self, routes: list[list[int]], cleaned: list[set[int]], already: set[int]
    ) -> bool:
        changed = False
        realized = [self.realize_route(route) for route in routes]
        for edge in self.ins.edges:
            if edge.category != "O" or edge.idx in already:
                continue
            best_vehicle = None
            best_gain = -INF
            for vehicle_idx, (_nodes, traversed, _time) in enumerate(realized):
                if edge.idx not in traversed:
                    continue
                capacity = CAPACITY[self.ins.vehicles[vehicle_idx]]
                if capacity < edge.requirement:
                    continue
    ...
    def fill_optionals(
        ...
        already = set().union(*cleaned) if cleaned else set()
        remaining = {
            edge.idx for edge in self.ins.edges if edge.category == "O"
        }
        remaining -= already

        if fast:
            top_k = 80 if self.is_large else 30
            while remaining:
                ranked = sorted(
                    remaining,
                    key=lambda edge_idx: self.estimate_optional_priority(edge_idx),
                    reverse=True,
                )
                scan_limit = len(ranked) if len(remaining) <= top_k else top_k
                best: tuple[tuple[float, ...], int, int, int, int] | None = None
                for edge_idx in ranked[:scan_limit]:
                    candidates = self.best_insertions(
                        routes, edge_idx, mandatory=False
                    )
                    if not candidates:
                        continue
                    gain = -candidates[0][0][1]
                    if gain <= 1e-15:
                        continue
                    candidate = (*candidates[0], edge_idx)
                    if best is None or candidate[0] < best[0]:
                        best = candidate
                if best is None:
                    break
                _key, vehicle_idx, position, _new_cost, edge_idx = best
                routes[vehicle_idx].insert(position, edge_idx)
                cleaned[vehicle_idx].add(edge_idx)
            ...
            while True:
                ...
                if best is None:
                    break
                _key, vehicle_idx, position, _new_cost = best
                routes[vehicle_idx].insert(position, best_edge)
                cleaned[vehicle_idx].add(best_edge)
                already.add(best_edge)
                remaining.remove(best_edge)
        # Mark no-time-cost optional streets only after routes stop changing;
        # otherwise a later insertion can change a shortest connector path.
        self.add_opportunistic(routes, cleaned, already)
        return routes, cleaned
    ...
    def solve(self, restarts: int = 250) -> Solution:
        ...
        if not self.is_large:
            ...
            if product_routes is not None:
                ...
                self.add_opportunistic_nodes(node_routes, product_cleaned)
            ...
            if exact_routes is not None:
                valid_mandatory_constructions += 1
                routes, cleaned = self.fill_and_compact(exact_routes, fast=False)
                routes, cleaned = self.refine_routes(routes, cleaned, rounds=2)
                solution = self.evaluate(routes, cleaned)
                if best is None or solution.score > best.score + 1e-12:
                    best = solution

        fast_optionals = self.is_large
        effective_restarts = min(restarts, 8) if self.is_large else restarts
        fill_cycles = 2 if self.is_large else 6

        for restart in range(effective_restarts):
            routes = self.construct_mandatory(restart)
            if routes is None:
                continue
            valid_mandatory_constructions += 1
            routes, cleaned = self.fill_and_compact(
                routes, fast=fast_optionals, max_cycles=fill_cycles
            )
            routes, cleaned = self.refine_routes(routes, cleaned, rounds=2)
            solution = self.evaluate(routes, cleaned)
            if best is None or solution.score > best.score + 1e-12:
...
def submission_text(instance: Instance, solution: Solution) -> str:
    ...
    for nodes, cleaned in zip(solution.routes, solution.cleaned):
        # Validator expects number of traversed streets, followed by k + 1 nodes.
